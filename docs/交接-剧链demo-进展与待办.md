# 交接：Adborn 投资人 Demo（原"剧链 DramaLink"，2026-08-03 定名）

> **定名记录**：2026-08-03 产品定名 **Adborn**（Ad+born，广告与内容一同出生），域名 adborn.ai 用户已购入（Namecheap，2 年）。备选产品线名"入画 FrameIn"（framein.ai 查询时未注册，用户未购）。下文历史记录中的"剧链 DramaLink"均为旧称。站内品牌文案已全局替换。

## 背景

用户（产品负责人）做 AI 短剧广告植入方向创业，需要给投资人看的可交互 demo。调研背景见 `~/Claude/reports/2026-07-22-AI短剧广告植入公司调研.html`。

商业定位（用户拍板）："AI 时代的广告平台"，AI 短剧是 AI 生成内容中变现潜力最大的第一站；类比 CJ/Awin 分佣 + Skimlinks 自动转链，**对投资人不提 MCN 类比**（估值不利）。

## 目标与现状（已上线真实素材版）

- 代码 `~/Claude/drama-ads-demo/`（GitHub `xinyuxinsheng/drama-ads-demo`），线上 https://xinyuxinsheng.github.io/drama-ads-demo/
- 三阶段按行业术语命名：**剧本开发·智能植入规划 / AI 成片·无痕植入生成 / 上线投流·全球转链变现**
- 剧本开发：五步流程条 + 剧本《她的耳机里没有歌》两场戏标 5 个植入位（数码/背具/服装/道具/场景）；商品卡三种形态——已匹配（Sony 耳机、Kånken 背包）、剧方候选二选一（adidas T恤 vs Samba，可点选）、**多品牌竞价**（扇面广告位：大众点评必吃榜 ¥52,000 中标 vs 高德扫街榜 ¥45,000 未中标）
- AI 成片：三组横屏对比，滚动进视口自动同步循环播放（IntersectionObserver）。第三组是**千人千面**：vb-original（空白山水扇原片）→ 右侧两个投放版本（vb-dianping 必吃榜给美食人群 / vb-gaode 扫街榜给出行人群），讲"同一广告位按人群反复售卖"；工作台竞价卡口径已联动（高德=定向份额）
- 投流变现：横屏播放器（va-placed），片头植入声明、弹幕（config 卡点）、Sony 商品卡 + 美/日/中转链（Amazon $278 / 楽天 ¥42,900 / 天猫 ¥1,899，示例价）
- 用户提供的真实素材：三对原片/植入后视频（横屏）+ 6 张商品图，均已入 assets/（对应关系见 README）
- 页脚已声明：所有品牌仅为演示示意，非真实合作

## 已验证（2026-07-23 本地）

候选切换/整案确认/步骤推进 ✓、三组对比自动播放 ✓、弹幕按卡点出现 ✓、商品卡 0.8s 弹出 ✓、三国切换 ✓、手机端竖排布局 ✓、无横向溢出 ✓、控制台无报错 ✓。

## 市场空间与竞争格局页（2026-08 新增）

- 市场空间页（投流变现后）：四卡——植入 $330 亿 / 达人营销 $2,500 亿 / 短剧 500 亿+ / Adborn 预估 7–15 亿（金色高亮，标注自行测算）。
- 竞争格局页（市场空间后、团队前，2026-08-11）：对比表——对话层 Gravity $3,850 万 / Koah $2,600 万 / Kontext $1,000 万（合计 ~$7,500 万，变现 AI 对话），内容层 Rembrand $3,100 万（拍完后贴入），Adborn 高亮行独占"生成前资产层植入+CPS"。口径："他们变现 AI 的对话，我们变现 AI 的内容"。用户拍板不放 InScene。数据来源见 reports/2026-08-11-AI原生广告竞争格局五家深调.html。

- 市场空间页风向标横条（2026-08-12）：三个巨头动作——字节 2026-04 成立"红果电商"独立部门（红果 DAU 破亿、边看剧买"短剧同款"，IT之家/21经济多源✅）、OpenAI 2026-02 ChatGPT 上广告（两个月年化 $1 亿📰）、Meta 2026-04 Reels 开放视频内联盟挂链（打通 Amazon📰）——验证"内容即卖场"。
- 想象空间页（竞争格局后、团队前，2026-08-12）：六宫格——AI 短剧（第一站·已演示，金色高亮）+ 五大扩张方向（AI 互动剧/互动游戏/音频/图片/文字），每格一句"广告长在哪里"。口径："内容形态在变，'广告与内容一同出生'不变"。

## 插件形态 demo（2026-08-01 新增）

用户拍板"生成即植入"插件战略后新增两个独立演示页（不在主页导航，直链访问）：
- `gen-plugin.html` 形态①已按用户拍板改为**资产层植入**（2026-08-03）：三步流程——①角色资产选品牌款（4 槽位：Sony/Kånken/adidas T恤/Samba，生成三视图 char-sheet.jpg 带槽位标注）→ ②场景/道具资产绑定（茶馆→霸王茶姬、扇面→必吃榜，图片直接换版本）→ ③用资产生成剧集（三条植入版视频，"植入自动在场"）。核心口径：**植入发生在视频生成之前，生成后再植入就晚了**；换广告主=换资产版本。
- `studio-plugin.html` 形态②承制方工作台：剪映式时间线 3 片段 + 植入工具包 4 位（¥84,000 保底）→ 一键应用（缩略图/预览换植入版）→ 收益对比条"承制费 ¥19.2万 + 植入增收 ¥8.4万(+44%)"。
- 战略口径：插件是获客楔子非壁垒（可灵/即梦母公司自带广告部门会抄），护城河在商家资产库+跨平台分佣结算网络。

## 待办 / 已知事项

1. **va 视频是两镜头剪辑**：商场镜头（Sony+Kånken 植入）后接天桥日落镜头（女主背吉他包、白色耳机）。若桥上镜头后续也做植入（吉他包→YAMAHA），可在 js/config.js 加第二张商品卡卡点。
2. 用户如需再换视频/改卡点：覆盖 assets/ 同名文件 + 改 js/config.js（playerVideo/badge/productAt/danmaku/products）。
3. 平台名"剧链 DramaLink"仍为占位名。
4. 上次 push 需用户亲自执行（权限分类器拦 gh repo create）；本次 git push 若再被拦，让用户跑：`cd ~/Claude/drama-ads-demo && git push`。

## 技术备注

- 纯静态 HTML/CSS/JS。视频总量约 22MB（GitHub Pages 单文件 100MB 内，安全）。
- 本地预览：launch.json `drama-ads-demo`（端口 8460）。浏览器面板截图有已知怪癖：视口尺寸需与截图面一致，视频层未播放时截图呈黑。
